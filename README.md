...
在命令模式下输入命令时，按 **Tab** 键可以自动补全命令和参数：

| 输入场景 | 补全内容 |
|----------|----------|
| 输入 `/` 后跟部分命令名（如 `/m`） | 补全为匹配的命令：`/memory`、`/model`、`/models` |
| 输入 `/memory ` 后跟子命令前缀 | 补全为匹配的子命令：`/memory show`、`/memory add`、`/memory clear` |
| 输入 `/model ` 后跟模型名片段 | 根据可用模型列表补全模型名称 |
| 输入 `/session ` 后跟会话 ID 片段 | 根据当前会话列表补全会话 ID |

连续按 **Tab** 可循环切换匹配的候选项，输入新字符或删除字符后补全列表自动重置。
